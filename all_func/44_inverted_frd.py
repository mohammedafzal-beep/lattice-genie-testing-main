...
def Inverted_FRD(C, a1,a2, resolution = 200, folder='all_files'):
    
    
    V, F = generate_iso_mesh(size, resolution, scale, C, kind='frd', mode='skeletal')
    V = snap_to_cube_planes(V, SNAP_TOL)
    V, F = decimate_and_clean(V, F, MAX_TRIS_FOR_STEP)
    V, F = build_end_caps(V, F, tol=SNAP_TOL, kind='frd', direction='reverse',mode='skeletal')
    rotate(V,a1,a2,0) 
    filename = f"44Inverted_Diamond_{C:.2f}_{a1}_{a2}.stl"  # Format filename with the c value
    ...
    scale = 2 * np.pi / size
    filename = f"44Inverted_FRD_{resolution}"
