...
def Skeletal_IWP(C, a1,a2, resolution = 200, folder='all_files'):
    
    V, F = generate_iso_mesh(size, resolution, scale, C, kind='iwp', mode='skeletal')
    V = snap_to_cube_planes(V, SNAP_TOL)
    V, F = decimate_and_clean(V, F, MAX_TRIS_FOR_STEP)
    V, F = build_end_caps(V, F, tol=SNAP_TOL, kind='iwp', direction='normal',mode='skeletal')
    rotate(V,a1,a2,0) 
    filename = f"39skeletal_TPMS_IWP_{C:.2f}_{a1}_{a2}.stl"  # Format filename with the c value
    ...
    scale = 2 * np.pi / size
    c_values = list(np.arange(-3, 3, 0.1)) + [-2.95, 2.95]  # Define the range for c
    # c_values = [1.9, 1.7]
    filename = f"39Skeletal_IWP_{C:.1f}_{resolution}.stl"
