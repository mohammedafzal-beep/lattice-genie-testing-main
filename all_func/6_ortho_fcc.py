...
import os
def Ortho_FCC(a, b, c,   overlap=0.01,a1=0,a2=0,face_atom_radius=0.4,r=0.47,resolution = 200, folder='all_files'):

    # Function to generate atomic positions in a cubic lattice
    ...
    def generate_solid_volume(resolution, positions, T, atom_radius, face_atom_radius, a, b, c, plane_equation):
        v1 = positions[1]  # Assuming positions passed are atom_positions
        v2 = positions[2]
        v3 = positions[3]
        ...
        # Vectorized grid transform
        parallelepiped_grid = (u[..., np.newaxis] * v1) + (v[..., np.newaxis] * v2) + (w[..., np.newaxis] * v3)
        x = parallelepiped_grid[..., 0]
        y = parallelepiped_grid[..., 1]
        z = parallelepiped_grid[..., 2]

        # Helper to compute bravais_function for a given atom position and radius
        def compute_values(pos, radius):
            return (x - pos[0]) ** 2 + (y - pos[1]) ** 2 + (z - pos[2]) ** 2 - radius ** 2

        # List of positions and their radii
        atom_radii_list = [
            (positions[i], atom_radius) for i in range(8)
        ] + [
            (positions[i], face_atom_radius) for i in range(8, 14)
        ]

        # Compute values in parallel
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(compute_values, pos, r) for pos, r in atom_radii_list]
            results = [f.result() for f in futures]

        # Minimum of all values arrays
        values = results[0]
        for val in results[1:]:
            values = np.minimum(values, val)

        # Apply plane equation mask (this can also be vectorized if needed)
        for normal_vector, D in plane_equation.values():
            ...
            values[mask] = 1  # Setting boundary values
    ...
    atom_radius_range = np.arange(0.47, 0.53, 0.01)  # Considered lower tolerance than 0.03 to generate atleast 4+ structures
    # face_atom_radius_range = np.arange(0.4, 0.48, 0.01) # if the tolernce put for both corner and face atoms is kept same, unable to  generate more than 4 structures with all atoms and voids connected. SO used a diff tolerance of 0.005 for face atoms like below.
    face_atom_radius_range = np.arange(0.4, 0.44, 0.005)  # Considered lower tolerance than 0.03 to generate atleast 4+ structures
    ...
    filename = f"6Ortho_FCC_{a:.1f}_{b:.1f}_{c:.1f}_{face_atom_radius:.2f}_{r:.2f}_{resolution}.stl" 
