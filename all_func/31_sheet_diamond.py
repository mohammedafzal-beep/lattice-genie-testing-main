...
def Sheet_Diamond(C, direction, a1,a2, resolution = 200, folder='all_files'):
    ...
    V, F = generate_iso_mesh(1, resolution, scale, C, kind='diamond', mode='sheet')

    V, F = decimate_and_clean(V, F, MAX_TRIS_FOR_STEP)
    V, F = build_end_caps(V, F, tol=SNAP_TOL, kind='diamond', direction=direction,mode='sheet')
    rotate(V,a1,a2,0) 
    filename = f"31Sheet_Diamond_{C:.2f}_{direction}_{a1}_{a2}.stl"  # Format filename with the c value
