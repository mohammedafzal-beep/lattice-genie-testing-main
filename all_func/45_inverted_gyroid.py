...
def Inverted_Gyroid(C, a1,a2, resolution = 200, folder='all_files'):
    
    
    V, F = generate_iso_mesh(size, resolution, scale, C, kind='g', mode='skeletal')
    ...
    rotate(V,a1,a2,0) 
    filename = f"45Inverted_TPMS_Gyroid_{C:.2f}_{a1}_{a2}.stl"  # Format filename with the c value
    ...
    scale = 2 * np.pi / size
    filename = f"45Inverted_Gyroid_{resolution}"
