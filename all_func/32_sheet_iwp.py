...
SNAP_TOL = 1e-6
scale = 2 * math.pi
def Sheet_IWP(C, direction, a1,a2, resolution = 200, folder='all_files'):
    
    
    V, F = generate_iso_mesh(1, resolution, scale, C, kind='iwp', mode='sheet')

    V, F = decimate_and_clean(V, F, MAX_TRIS_FOR_STEP)
    V, F = build_end_caps(V, F, tol=SNAP_TOL, kind='iwp', direction=direction,mode='sheet')
    rotate(V,a1,a2,0) 
