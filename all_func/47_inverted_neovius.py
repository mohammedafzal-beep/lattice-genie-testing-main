...
def Inverted_Neovius(C, a1,a2, resolution = 200, folder='all_files'):
    ...
    V, F = build_end_caps(V, F, tol=SNAP_TOL, kind='n', direction='reverse',mode='skeletal')
    rotate(V,a1,a2,0) 
    filename = f"47Inverted_Neovius_{C:.2f}_{a1}_{a2}.stl"  # Format filename with the c value
    ...
    def create_and_invert_stl(verts, faces, folder, filename_base, c):
        ...
        inverted_filename = f"{filename_base}_inverted_c_{c}.stl"
    ...
    scale = 2 * np.pi / size
    filename = f"47Inverted_Neovius_{resolution}"
