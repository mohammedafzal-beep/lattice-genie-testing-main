...
import os
def Inverse_Rhombo(a, b, c, r, alpha, beta, gamma,    resolution = 200, folder='all_files'):
    ...
    def generate_solid_volume(resolution, position, T, r, a, b, c, plane_equation):
        ...
        values = np.minimum.reduce([
            bravais_function(x, y, z, positions[i], r) for i in range(8)
        ])
    ...
    atom_radius_range = np.arange(0.78, 0.99, 0.03)
    ...
    filename = f"27Inverse_Rhombo_{a:.1f}_{b:.1f}_{c:.1f}_{r:.2f}_{alpha}_{beta}_{gamma}_{resolution}.stl"
